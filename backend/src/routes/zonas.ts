...
router.get('/distritos', authMiddleware, async (_req: Request, res: Response) => {
  try {
    const { rows } = await pool.query('SELECT id, nombre FROM distritos ORDER BY nombre');
    res.json(rows);
  } catch {
    res.status(500).json({ error: 'Error' });
  }
...
router.post('/distritos', authMiddleware, adminOSupervisor, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO distritos (nombre) VALUES ($1)
       ON CONFLICT (nombre) DO UPDATE SET nombre=EXCLUDED.nombre
       RETURNING id, nombre`,
      [nombre]
    );
