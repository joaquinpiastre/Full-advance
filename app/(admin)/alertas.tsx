...
type Filtro = 'todas' | 'urgentes' | 'vencimientos';
...
function CardAlerta({ item }: { item: Alerta }) {
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.85}
      onPress={toggle}
      style={[styles.card, esUrgente ? styles.cardUrgente : styles.cardVenc]}
    >
      {/* Fila principal siempre visible */}
      <View style={styles.cardRow}>
        <View style={[styles.tipoPill, esUrgente ? styles.pillUrgente : styles.pillVenc]}>
          <Text style={styles.pillTexto}>{esUrgente ? '🚨 URGENTE' : '📦 VENC.'}</Text>
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabs}>
        {([
          { key: 'todas', label: 'Todas', count: urgentes.length + vencimientos.filter((v) => !v.urgente).length },
      ...
      {filtro === 'vencimientos' ? (
        ...
        <FlatList
          data={lista}
          keyExtractor={(item) => `${item.urgente ? 'u' : 'v'}-${item.id}`}
          contentContainerStyle={styles.lista}
          refreshControl={
